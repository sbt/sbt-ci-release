...
class CiReleaseTest extends munit.FunSuite {
  ...
  test("Command starting with number is assumed to be a cross version") {
    assertEquals(
      backPubVersionToCommand("1.1.0@2.12.20"),
  ...
  test("Commands can be chained") {
    assertEquals(
      backPubVersionToCommand("1.1.0@2.12.20@foo/publishSigned"),
      ";++2.12.20!;foo/publishSigned"
    )
    assertEquals(
      dropBackPubCommand("1.1.0@2.12.20@foo/publishSigned"),
      expectedVer
    )
  ...
  test("Treat # as comments") {
    ...
    assertEquals(
      backPubVersionToCommand("1.1.0@2.12.20#comment"),
      ";++2.12.20!;publishSigned"
    )
    assertEquals(dropBackPubCommand("1.1.0@2.12.20#comment"), expectedVer)

    assertEquals(
      backPubVersionToCommand("1.1.0@3.x#comment"),
      ";++3.x;publishSigned"
    )
